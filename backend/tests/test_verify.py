def test_verify_success(client, auth_headers):
    body = {
    ...
    response = client.post("/verify/", headers=auth_headers, json=body)
...
def test_verify_empty_context_chunks(client, auth_headers):
    body = {
...
def test_verify_empty_answer(client, auth_headers):
...
def test_verify_missing_api_key(client):
    body = {"answer": "Some claim here that is long enough to be split.", "context_chunks": []}
    response = client.post("/verify/", headers={"x-session-id": "test-session"}, json=body)
