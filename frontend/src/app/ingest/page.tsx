...
import { useUploadDocument, useIngestUrl, useIngestText, useGenerateSummary } from "@/lib/queries/documents";
...
export default function IngestPage() {
  ...
  const jobQuery = useJob(jobId, { enabled: stage === "chunking" });

  const stageIdx   = STAGES.indexOf(stage);
  const isRunning  = stage !== "idle" && stage !== "done" && stage !== "error";

  useEffect(() => {
    if (stage !== "chunking" || !jobQuery.data) return;
    if (jobQuery.data.status === "complete") {
      setStage("embedding");
      setStage("done");
      const result = jobQuery.data.result || {};
      const srcText = result.preview || (mode === "text" ? text.trim() : "");
      const srcName = result.source || "Document";
      if (srcText) {
        summaryMutation.mutate(
          { text: srcText, source: srcName },
          { onSuccess: (data) => setSummary(data.summary) }
        );
      }
    } else if (jobQuery.data.status === "failed") {
      setErrMsg(jobQuery.data.error || "Ingestion failed.");
      setStage("error");
    }
  }, [jobQuery.data, stage, mode, text]);

  useEffect(() => {
    if (jobQuery.isError && stage === "chunking") {
      setErrMsg(jobQuery.error?.message || "Job status check failed.");
      setStage("error");
    }
  }, [jobQuery.isError, jobQuery.error, stage]);

  const runIngest = useCallback(() => {
    setStage("uploading"); setSummary(null); setErrMsg(""); setJobId(null);

    const onSuccess = (data: { job_id: string }) => {
      setStage("chunking");
      setJobId(data.job_id);
    };
    const onError = (e: Error) => {
      setErrMsg(e.message || "Failed. Is the backend running on port 8000?");
      setStage("error");
    };

    if (mode === "file" && file) {
      setStage("extracting");
      uploadMutation.mutate(file, { onSuccess, onError });
    } else if (mode === "url" && url.trim()) {
      setStage("extracting");
      ingestUrlMutation.mutate(url.trim(), { onSuccess, onError });
    } else if (mode === "text" && text.trim()) {
      setStage("extracting");
      ingestTextMutation.mutate({ text: text.trim(), source: "Manual Input" }, { onSuccess, onError });
    } else {
      setErrMsg("Please provide a file, URL, or text first."); setStage("error");
    }
  }, [mode, file, url, text, uploadMutation, ingestUrlMutation, ingestTextMutation]);
