...
const btnPrimary =
  "inline-flex items-center justify-center rounded-md bg-neutral-950 px-5 py-2.5 text-[13.5px] font-semibold text-neutral-0 no-underline shadow-sm transition-opacity hover:opacity-90";
const btnSecondary =
  "inline-flex items-center justify-center rounded-md border border-neutral-300 bg-neutral-0 px-5 py-2.5 text-[13.5px] font-medium text-neutral-950 no-underline transition-colors hover:border-neutral-950";

export default function LandingPage() {
  const [menuOpen, setMenuOpen] = useState(false);

  return (
    <div className="min-h-screen w-full overflow-x-hidden bg-neutral-50 font-sans text-neutral-950">
      <style>{`
      ...
      <header className="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-neutral-200 bg-neutral-50/85 px-5 backdrop-blur-md sm:px-8 lg:px-14">
        <Link href="/" className="flex items-center gap-2.5 no-underline">
          <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-md bg-neutral-950">
        ...
        <nav className="hidden items-center gap-9 md:flex">
          {NAV_LINKS.map((l) => (
            <a
              key={l.label}
              href={l.href}
              className="font-mono text-[12px] uppercase tracking-[0.08em] text-neutral-600 no-underline transition-colors hover:text-neutral-950"
            >
              {l.label}
            </a>
          ))}
        ...
        <button
          ...
          className="flex h-9 w-9 items-center justify-center rounded-md border border-neutral-300 text-neutral-950 md:hidden"
        >
      ...
      {menuOpen && (
        <div className="fixed inset-x-0 top-16 z-40 flex flex-col gap-4 border-b border-neutral-200 bg-neutral-50 px-5 pb-7 pt-5 sm:px-8 md:hidden">
          {NAV_LINKS.map((l) => (
            <a
              key={l.label}
              href={l.href}
              onClick={() => setMenuOpen(false)}
              className="text-[15px] text-neutral-950 no-underline"
            >
              {l.label}
            </a>
          ))}
      ...
      <section className="relative overflow-hidden px-5 pb-24 pt-20 sm:px-8 lg:px-14 lg:pt-28">
        <div
          ...
          style={{
            background:
              "radial-gradient(ellipse 60% 55% at 50% 0%, rgba(91,94,244,0.09), transparent 70%), radial-gradient(ellipse 45% 40% at 90% 15%, rgba(212,98,42,0.05), transparent 60%)",
          }}
        />
        ...
        <div className="relative mx-auto grid max-w-[1220px] gap-16 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
          <motion.div {...fadeUp(0)}>
            ...
            <p className="mt-6 max-w-[490px] text-[16px] leading-[1.7] text-neutral-700">
            ...
            <div className="mt-14 grid max-w-[560px] grid-cols-2 gap-y-6 border-t border-neutral-200 pt-6 sm:grid-cols-4">
              {NUMBERS.map(([val, label], i) => (
                <div key={label} className={i > 0 ? "border-neutral-200 pl-5 sm:border-l" : ""}>
                  <div className="font-mono text-[19px] font-medium tracking-[-0.01em] text-neutral-950">{val}</div>
                  <div className="mt-1.5 font-mono text-[10px] uppercase tracking-[0.08em] text-neutral-600">{label}</div>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div {...fadeUp(0.1)} className="relative">
            <div className="overflow-hidden rounded-lg border border-neutral-200 bg-neutral-0 shadow-premium">
              <div className="flex items-center gap-2 border-b border-neutral-200 bg-neutral-50 px-4 py-3">
                <span className="h-2.5 w-2.5 rounded-full bg-neutral-300" />
                <span className="h-2.5 w-2.5 rounded-full bg-neutral-300" />
                <span className="h-2.5 w-2.5 rounded-full bg-neutral-300" />
                <span className="ml-2 rounded-full bg-neutral-100 px-3 py-1 font-mono text-[10.5px] text-neutral-600">
                  prism.app/workspace
                </span>
              </div>

              <div className="flex flex-col gap-4 p-5">
                <div className="ml-auto max-w-[78%] rounded-xl rounded-tr-sm bg-neutral-950 px-4 py-2.5 text-[13px] text-neutral-0">
                  What did the ablation study find?
                </div>

                <div className="max-w-[92%] rounded-xl rounded-tl-sm border border-neutral-200 bg-neutral-50 px-4 py-3 text-[13px] leading-relaxed text-neutral-800">
                  Removing the reranking stage reduced retrieval precision by a measurable margin
                  <span className="font-mono font-semibold text-brand-600">[1]</span>, while the
                ...
                <div className="flex flex-wrap gap-2">
                  <span className="inline-flex items-center gap-1.5 rounded-md border border-neutral-200 bg-neutral-0 px-2.5 py-1 font-mono text-[10.5px] text-neutral-600">
                ...
                <div className="mt-1 grid grid-cols-3 gap-2 border-t border-neutral-200 pt-4">
                  <div className="rounded-md bg-success-50 px-3 py-2">
                    <div className="font-mono text-[15px] font-bold text-success-600">82%</div>
                    <div className="font-mono text-[9px] font-bold uppercase tracking-wide text-success-600/80">Supported</div>
                  </div>
                  <div className="rounded-md bg-accent-50 px-3 py-2">
                    <div className="font-mono text-[15px] font-bold text-accent-600">12%</div>
                    <div className="font-mono text-[9px] font-bold uppercase tracking-wide text-accent-600/80">Uncertain</div>
                  </div>
                  <div className="rounded-md bg-neutral-100 px-3 py-2">
                    <div className="font-mono text-[15px] font-bold text-neutral-700">6%</div>
                    <div className="font-mono text-[9px] font-bold uppercase tracking-wide text-neutral-600">Unsupported</div>
                  </div>
                </div>

                <div className="flex items-end gap-1.5 border-t border-neutral-200 pt-4">
                  {CHART_BARS.map((h, i) => (
                    <div
                      key={i}
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          {FEATURES.map((f, i) => (
            <motion.div
              key={f.t}
              {...fadeUp(i * 0.05)}
              className={`flex min-h-[150px] flex-col gap-3 rounded-lg border border-neutral-200 bg-neutral-0 p-6 shadow-xs ${
            ...
            >
              ...
              <div className="text-[14.5px] font-semibold text-neutral-950">{f.t}</div>
              <div className="text-[13px] leading-[1.6] text-neutral-700">{f.d}</div>
            </motion.div>
          ))}
        </div>
      </section>

      <section id="product" className="border-t border-neutral-200 bg-neutral-0 px-5 py-24 sm:px-8 lg:px-14">
          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {PRODUCT_PAGES.map((p, i) => (
              <motion.div
                key={p.label}
                {...fadeUp(i * 0.04)}
                className="flex flex-col gap-3 rounded-lg border border-neutral-200 bg-neutral-50 p-6"
              >
                <p.icon size={16} className="text-neutral-600" />
                <div className="text-[13.5px] font-semibold text-neutral-950">{p.label}</div>
                <div className="text-[12px] leading-[1.6] text-neutral-700">{p.d}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section
        id="workflow"
        className="flex justify-center border-t border-neutral-200 px-5 py-24 text-center sm:px-8 lg:px-14"
      >
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3.5 border-t border-neutral-200 bg-neutral-0 px-5 py-6 sm:px-8 lg:px-14">
        <div className="flex items-center gap-2">
          <div className="flex h-6 w-6 items-center justify-center rounded-md bg-neutral-950">
            <Zap size={12} color="#ffffff" strokeWidth={2.5} />
          </div>
          <span className="font-display text-[15px] text-neutral-600">Prism</span>
        </div>
        ...
        <span className="font-mono text-[11px] text-neutral-400">Research Intelligence Platform</span>
      </footer>
    </div>
  );
}